...
- `Metal/MetalRenderBackend.*` 在 macOS 上协调 `MTLDevice`、command queue 和 `CAMetalLayer` 资源，并执行基础 clear/present、渐变背景、深度测试、主网格三角面、普通边线/点绘制、云图标量映射、部件显隐、Element 离屏拾取、选中高亮线、左下角坐标轴、叠加线框、切片交线、半透明等值面和裁剪预览。
...
- `Metal/MetalLayerHost.*` 负责从 `QWindow` 获取/配置 `CAMetalLayer`，并维护 drawable size。
...
负责创建系统默认 `MTLDevice`、command queue，并绑定 `QWindow` 原生
`NSView/CAMetalLayer` 执行基础 clear/present、渐变背景、深度测试、主网格三角面、普通边线、点绘制、云图标量映射、选中高亮线、左下角坐标轴、叠加线框、切片交线、半透明等值面和裁剪预览；
