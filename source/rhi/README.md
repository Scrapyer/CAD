...
- `Metal/`：Metal 后端骨架、设备信息探测、基础 clear/present、渐变背景、深度测试、主网格三角面、普通边线/点绘制、云图标量映射、部件显隐、Node/Element/Part 点选/框选、选中高亮、左下角坐标轴、叠加线框、切片交线、等值面和裁剪预览。

- `RenderBackend.h` 定义 `IRenderBackend` 和后端信息结构。
- `RenderBackendFactory.*` 提供后端创建入口，按构建选项返回 OpenGL、Vulkan 或 Metal 后端。
...
- `Metal/MetalRenderBackend.*` 在 macOS 上协调 `MTLDevice`、command queue 和外部宿主提供的 `CAMetalLayer` 资源，并执行基础 clear/present、渐变背景、深度测试、主网格三角面、普通边线/点绘制、云图标量映射、部件显隐、Element 离屏拾取、选中高亮线、左下角坐标轴、叠加线框、切片交线、半透明等值面和裁剪预览。
- `Metal/MetalAttachmentResourceBuilder.*` 负责确保主 depth attachment、overlay/depth state、离屏 pick color/depth attachment 和 pick readback buffer。
- `Metal/MetalBufferResource.*` 封装 Metal `MTLBuffer` 生命周期，当前覆盖主网格、普通边线、点显示、overlay、slice、selection、坐标轴、等值面、裁剪预览和 pick readback buffer。
- `Metal/MetalClearFramePass.*` 负责 Metal clear/present command buffer 提交。
- `Metal/MetalDepthStencilFactory.*` 封装 depth-stencil descriptor 配置和 state 创建。
- `Metal/MetalDeviceFactory.*` 负责系统默认 `MTLDevice`、command queue 和 `RenderBackendInfo` 创建。
- `Metal/MetalDrawableFrameSubmitter.*` 负责主 drawable frame 的 command buffer/encoder 创建、mesh pass 录制调用和 present/commit。
- `source/app/platform/macos/MacOSMetalLayerHost.*` 负责从 `QWindow` 获取/配置 `NSView/CAMetalLayer`，并维护 drawable size；Metal 后端只接收已准备好的 layer 和 drawable size。
- `Metal/MetalLineUpload.*` 封装 overlay、slice、selection 和裁剪预览边框线段上传的校验与 buffer 写入。
- `Metal/MetalMeshFramePass.*` 负责 Metal 主视口 command encoder 内的背景、mesh、edge、point、overlay、slice、iso、clip、selection 和 axes draw 录制。
- `Metal/MetalMeshFramePassBuilder.*` 负责构建主视口 uniform set，并把 draw flags、资源句柄和 uniform 组装为 `MetalMeshFramePassInputs`。
- `Metal/MetalMeshScalarUpdater.*` 负责 shared mesh vertex buffer 中云图 scalar 字段的局部更新。
- `Metal/MetalMeshUploadBuilder.*` 负责按部件显隐、拾取 ID、顶点色和云图标量生成 Metal 主网格/点/边线上传数据。
- `Metal/MetalMeshUploader.*` 负责把 Metal 主网格、点显示和普通边线 CPU 上传数据写入 Metal buffer，并同步绘制计数。
- `Metal/MetalObjectResource.*` 封装 Metal 外层 Objective-C 对象生命周期，当前覆盖 `MTLDevice`、`MTLCommandQueue` 和 `CAMetalLayer`。
- `Metal/MetalPipelineFactory.*` 封装运行时 MSL shader 编译、vertex layout 配置和 render pipeline state 创建与资源确保。
- `Metal/MetalPickPass.*` 负责 Metal 离屏拾取 draw、1x1 blit readback copy 和 command buffer 等待。
- `Metal/MetalPickPassBuilder.*` 负责把离屏拾取资源句柄、坐标和 uniform 组装为 `MetalPickPassInputs`。
- `Metal/MetalPickUtils.*` 封装拾取 ID 颜色编码、解码和单像素 readback buffer 读取。
- `Metal/MetalRenderPassFactory.*` 封装 clear、主视口 mesh 和离屏 pick render pass descriptor 配置。
- `Metal/MetalShaderSources.*` 集中保存 Metal 后端运行时编译使用的 MSL shader 源码。
- `Metal/MetalShaderTypes.h` 集中保存 C++ 侧与 MSL vertex/uniform layout 对齐的结构体。
- `Metal/MetalStateResource.*` 封装 Metal pipeline/depth-stencil state 生命周期，当前覆盖背景、主网格、等值面、线段、pick pipeline 和 depth-stencil state。
- `Metal/MetalSurfaceUploadBuilder.*` 负责等值面和裁剪/切片平面预览三角面上传数据构建。
- `Metal/MetalSurfaceUploader.*` 负责把等值面、裁剪/切片平面预览三角面和裁剪预览边框线写入 Metal buffer，并同步绘制计数。
- `Metal/MetalTextureResource.*` 封装 Metal `MTLTexture` 生命周期，当前覆盖主视口 depth attachment 和离屏 pick color/depth attachment。
- `Metal/MetalUniformUtils.*` 封装 mesh/line/pick 绘制使用的 MVP、颜色和云图参数 uniform 构建。
...
- 后续 Metal mesh pipeline / QRhi 后端也放到对应子目录，避免继续扩大 `source/render/GLWidget.*`。
...
Metal 后端通过 `FEMODELVIEWER_ENABLE_METAL_RHI` 控制，默认在 macOS 上查找
`Metal.framework`、`Foundation.framework`、`AppKit.framework` 和 `QuartzCore.framework`
后编译。当前 `MetalRenderBackend`
负责创建系统默认 `MTLDevice`、command queue，并绑定由 macOS 平台层准备好的
`CAMetalLayer` 执行基础 clear/present、渐变背景、深度测试、主网格三角面、普通边线、点绘制、云图标量映射、选中高亮线、左下角坐标轴、叠加线框、切片交线、半透明等值面和裁剪预览；
`MetalViewport` 已可作为 `RenderViewport` 的启动期宿主，并接收 `setMesh()`、
`setObjectColor()`、`setModelDisplayMode()`、`setVertexScalars()`、`setTriangleToPartMap()`、`setEdgeToPartMap()`、
...
当前阶段已抽离后端边界、通用 `Scene*` pass 描述、OpenGL 上下文信息、shader program 创建、scene/axes shader uniform 设置、默认 OpenGL 状态、基础资源创建、主网格 VAO/VBO/IBO/颜色/标量缓冲托管、普通边线 VAO/VBO/IBO 托管、选中高亮/轮廓边 VAO/VBO 托管、叠加线框、切片交线、等值面、裁剪/切片平面预览 VAO/VBO/IBO 托管、部件索引 texture buffer 资源托管、常规 VAO/VBO/IBO 上传、固定 position+color 几何资源托管、拾取 framebuffer 和拾取 VAO 托管、基础 viewport/clear/depth/blend/cull 状态切换、pass 状态应用、聚合 scene pass 执行、托管资源 scene pass 执行、常规 `drawArrays` / `drawElements` 调用，以及拾取绘制和像素读取。Vulkan 路径已经开始把裸 buffer 生命周期收敛到 `VulkanBufferResource`，当前覆盖主网格、普通边线、scalar SSBO、overlay、slice、selection line、iso surface、clip preview 和 pick readback；主网格、普通边线、iso surface、clip preview 三角面和动态线几何已从长期 host-visible buffer 改为 staging 上传到 device-local buffer，主 mesh / iso surface / clip preview 的多段 staging copy 已通过 `VulkanStagingUploadContext` 合并为单次 command buffer + fence 提交；主 mesh/edge/scalar/descriptor 已收敛到 `VulkanMeshBufferResources`，frame pipeline 句柄已收敛到 `VulkanFramePipelines`；主 depth 已收敛到 `VulkanDepthResource`，pick color/depth/framebuffer/readback 已收敛到 `VulkanPickResources`，swapchain image view/framebuffer 集合已收敛到 `VulkanSwapchainFrameResources`；scalar descriptor set layout 已收敛到 `VulkanDescriptorSetLayoutResource`，主视口和拾取路径的 render pass / framebuffer 已收敛到 `VulkanRenderPassResource` / `VulkanFramebufferResource`，graphics pipeline 和 pipeline layout 已收敛到 `VulkanPipelineResource`，主 command pool / command buffer 已收敛到 `VulkanCommandResource`，mesh frame 内的 iso/clip/overlay/edge/slice/selection draw 录制已收敛到 `VulkanMeshFramePass`，拾取绘制和 readback barrier/copy 录制已收敛到 `VulkanPickPass`。Metal 路径已开始把 attachment 确保逻辑收敛到 `MetalAttachmentResourceBuilder`，把裸 `MTLBuffer` / `MTLTexture` / state object / 外层 ObjC 对象生命周期收敛到 `MetalBufferResource` / `MetalTextureResource` / `MetalStateResource` / `MetalObjectResource`，把系统默认 device/command queue 创建收敛到 `MetalDeviceFactory`，把 `QWindow` 到 `NSView/CAMetalLayer` 的宿主配置收敛到 `MacOSMetalLayerHost`，`MetalRenderBackend` 只接收已准备好的 layer 和 drawable size，并把 shader 源码收敛到 `MetalShaderSources`，把 C++/MSL 共享布局收敛到 `MetalShaderTypes`，把 shader 编译、render pipeline 创建和资源确保收敛到 `MetalPipelineFactory`，把 render pass descriptor 创建收敛到 `MetalRenderPassFactory`，把 uniform 构建收敛到 `MetalUniformUtils`，把 pick 颜色编解码和 readback 读取收敛到 `MetalPickUtils`，把 mesh 上传数据构建收敛到 `MetalMeshUploadBuilder`，把主 mesh/point/edge buffer 上传和计数同步收敛到 `MetalMeshUploader`，把 mesh scalar 局部更新收敛到 `MetalMeshScalarUpdater`，把等值面/裁剪预览上传数据构建收敛到 `MetalSurfaceUploadBuilder`，把等值面/裁剪预览 buffer 上传和计数同步收敛到 `MetalSurfaceUploader`，把动态线段上传收敛到 `MetalLineUpload`，把 clear/present 提交收敛到 `MetalClearFramePass`，把主 drawable frame 提交收敛到 `MetalDrawableFrameSubmitter`，把主视口 draw pass 输入组装收敛到 `MetalMeshFramePassBuilder`，把主视口 draw 录制收敛到 `MetalMeshFramePass`，把离屏拾取 pass 输入组装收敛到 `MetalPickPassBuilder`，把离屏拾取 draw/readback 录制收敛到 `MetalPickPass`，并把 depth-stencil state 创建收敛到 `MetalDepthStencilFactory`；当前覆盖主网格、普通边线、点显示、overlay、slice、selection、坐标轴、等值面、裁剪预览、主 depth、pick color/depth、pick readback、pipeline、render pass descriptor、uniform、mesh upload builder、mesh uploader、mesh scalar updater、surface upload builder、surface uploader、line upload、clear frame pass、drawable frame submitter、主视口 draw pass builder、主视口 draw pass、pick pass builder、pick pass、depth-stencil state、device factory、macOS layer host、device、command queue 和 layer 资源。

`RenderBackend.h` 放置与具体图形 API 无关的描述结构，例如 `RenderBackendKind`、`SceneFrameUniforms`、`SceneDrawUniforms`、`ScenePassState`、`ScenePrimitive` 和 `PickDrawItem`。`RenderSettings` 记录用户首选 RHI，当前支持保存 OpenGL、Vulkan 和 Metal 选项；`RenderViewport` 是应用层依赖的视口宿主，按全局设置在 `GLWidget`、macOS `VulkanViewport` 和 macOS `MetalViewport` 之间切换。Metal 后端已可在 macOS 上创建默认 `MTLDevice`、command queue，并通过 `CAMetalLayer` 执行基础 clear/present、渐变背景、深度测试、主网格三角面、普通边线、点绘制、云图标量映射、部件颜色/显隐、Node/Element/Part 点选/框选、选中高亮、左下角坐标轴、未变形叠加线框、基础切片交线、半透明等值面、裁剪/切片平面预览和轨道相机交互。`GLWidget` 通过 `createRenderBackend()` 创建当前视口可用后端，`OpenGLRenderBackend` 负责把这些通用描述映射到具体 OpenGL 常量和调用。

`GLWidget` 仍负责 OpenGL 路径的 Qt 生命周期、交互状态、矩阵计算、渲染顺序和完整拾取业务；`VulkanViewport` 负责 Vulkan 路径的原生窗口、surface/swapchain 生命周期、resize 或 acquire/present 返回 out-of-date/suboptimal 后的 swapchain 重建、主网格 present、左下角坐标轴和标签、Node / Element / Part 点选、框选添加/取消、部件轮廓/特征边高亮、节点标记、shader 端基础云图映射、未变形 overlay 线框、基础切片交线、半透明等值面叠加和裁剪/切片平面预览，`RenderViewport` 负责 Vulkan 路径的色标 overlay。`MetalViewport` 负责 Metal 路径的原生窗口、CAMetalLayer 生命周期、主网格 present、左下角坐标轴和标签、Node / Element / Part 点选、框选添加/取消、部件轮廓/特征边高亮、节点标记、shader 端基础云图映射、未变形 overlay 线框、基础切片交线、半透明等值面叠加和裁剪/切片平面预览。`OpenGLRenderBackend` 负责具体 OpenGL 资源创建、顶点属性绑定、GPU 数据上传、uniform 写入、pass 状态执行、聚合 pass 调度、基础绘制命令、主网格资源、普通边线资源、选中高亮资源、叠加线框、切片交线、等值面、裁剪/切片平面预览资源、背景/坐标轴这类固定几何资源，以及拾取 framebuffer / VAO 的创建和底层状态保护。常规 scene pass 已可直接接收后端托管资源，`GLWidget` 不再需要取出 VAO 指针。
