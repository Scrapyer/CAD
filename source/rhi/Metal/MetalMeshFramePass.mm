...
void bindMetalUniforms(id<MTLRenderCommandEncoder> encoder, const MetalMeshUniforms& uniforms)
{
                      ...
                      MTLPrimitiveType primitiveType)
{
    ...
    bindMetalUniforms(encoder, draw.uniforms);
...
void drawMetalAxes(id<MTLRenderCommandEncoder> encoder, const MetalMeshFramePassInputs& inputs)
{
    ...
    for (NSUInteger axis = 0; axis < 3; ++axis) {
        setMetalUniformColor(axesUniforms,
                             ...
                             axisColors[axis][2],
                             axisColors[axis][3]);
        bindMetalUniforms(encoder, axesUniforms);
...
void encodeMetalMeshFramePass(const MetalMeshFramePassInputs& inputs)
{
    ...
    [encoder setDepthStencilState:static_cast<id<MTLDepthStencilState>>(inputs.depthStencilState)];
    drawMetalIndexed(encoder, inputs.surface, MTLPrimitiveTypeTriangle);
    drawMetalIndexed(encoder, inputs.edges, MTLPrimitiveTypeLine);
    drawMetalVertices(encoder, inputs.points, MTLPrimitiveTypePoint);
    drawMetalIndexed(encoder, inputs.isoSurface, MTLPrimitiveTypeTriangle);
    drawMetalIndexed(encoder, inputs.clipPreview, MTLPrimitiveTypeTriangle);
    drawMetalVertices(encoder, inputs.overlay, MTLPrimitiveTypeLine);
    drawMetalVertices(encoder, inputs.slice, MTLPrimitiveTypeLine);
    drawMetalVertices(encoder, inputs.clipPreviewLines, MTLPrimitiveTypeLine);
    drawMetalVertices(encoder, inputs.selection, MTLPrimitiveTypeLine);
